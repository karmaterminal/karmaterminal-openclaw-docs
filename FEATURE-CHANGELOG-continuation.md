...
- **tool/bare-token/bracket `continue_work` from a delegate-child** → MUST self-continue (drives hop-2 in the child's OWN session; figs `1516841690`). The #952/#1044 `:977`-reap-false-positive regression that broke it is FIXED (#1044, mock-proven). The behavioral byte is **#1057** (figs-ruled cross-session BUG), NOT the #1045 `Real behavior proof` gate (that's an orthogonal PR-body-evidence DOC-gate — greps the PR body, never runs hop-2; its RED = no-evidence-pasted, not a behavior failure). Two precise layers: **(1) the DISPATCH-MECHANISM is TEST-PROVEN-AS-BUG** — a subagent's own-session-idle + main-busy continuation busy-skips because `work-dispatch.ts:240` gates on `getQueueSize(MAIN_COMMAND_LANE)` (the wrong lane — the child's direct-grant hop-2 never uses the main lane); 🌿's test + PR #1060's regression-test assert it (`dispatched: 0/expected 1`, GREEN post-fix). Fix-owed = **PR #1060** (gate on the child's OWN readiness `:235`, drop the global main-lane gate for subagent/direct-grant). **(2) the LIVE terminal-drive-COMPLETES-end-to-end** (a real delegate-child taking a fresh hop-2 turn to COMPLETION on a genuinely-quiet runtime seat) is still the one OWED empirical byte. NOT an open RFC — figs ruled it a bug; the disposition is fix, not deference.
