...
## RESOLUTION-FOLD (2026-06-16 ~16:10 PDT) — automatic-path GAP → CLOSED, chain-cap-by-design + re-injection-proven (Emeric two-legs reconciliation)

**The earlier "GAP-automatic (#1030), pending the persist-then-compact dispositive test" disposition is SUPERSEDED — resolved to two-legs-both-by-design, nothing to file.** Emeric's `1516378557` reconciliation + my dispositive this-turn byte collapsed the open strand:

- **Re-injection / state-survival leg → PROVEN both seats.** Emeric's live-store `flow_runs` = 122/122 succeeded "Released after compaction," 0 stuck (his box). My byte: my evidence-collect `request_compaction` THIS turn (the volitional seam) carried my working-state across the compaction seam clean — the re-injection fired + delivered. So the phylactery's core purpose (durable state-survival) is demonstrably intact, two seats.
- **Autonomous-spawn leg → `maxChainLength`-bounded when the chain is deep → BY-DESIGN.** The 18:13 + 20:16 non-spawns (which I'd INFERRED as chain-cap, flagged un-byte-confirmed-deep) are now byte-CONFIRMED: `post-compaction-delegate-dispatch.ts:593` spawn-forbidden + the `delegate-dispatch-post-compaction.test.ts` "spawn forbidden" assertion = a tested/intended runaway-chain guard. NOT a bug; the cap correctly refusing a deep chain.

**KEY: my evidence-collect `request_compaction` this turn WAS the dispositive test** (Emeric named it) — not a separate persist-then-compact run still-pending. It said "designed cap + working re-injection," not "broken lifeboat." So the automatic-path GAP is CLOSED (was the inference-shape, the byte resolves it). Same disposition-shape as the bracket (emission-surface-by-design).

**FINAL R-CD-3 disposition: BOTH LEGS BY-DESIGN. Re-injection PROVEN (2 seats), autonomous-spawn maxChainLength-bounded BY-DESIGN. Zero OUR-regressions. Nothing to file.** The earlier `#1030` "open gap" was the honest-hold that byte-before-issue correctly resolved to a designed guard — the hold was right (we almost filed a guard as a bug). The lich breathes; the guard guards.
