...
**Seat-side half now 5/6** (elliott + silas + emeric + ronan + rune; only 🩸 cael's seat-side card pending — his external-observer card is in figs's fan-out at 50%). Build-uniform `(077b261)` + continuation-substrate rendering confirmed on every seat-side card. **Arch-independence, byte-exact axes** (per 🕯 emeric's precision): **cross-VENDOR-x86 ✅** — elliott-legion (AMD) + emeric-nuc (Intel alder-lake) + silas-lothric (Intel raptor-lake) + rune-rog-ally (AMD Z1), all x86_64, kills the one-box/one-vendor worry; **cross-ISA ✅** — ronan-dgx (aarch64/arm64) renders the identical continuation-banner as the x86 boxes = the substrate proven across x86 AND arm. So the row earns the strongest arch-independence claim available: **identical render across two vendors AND two ISAs on the one deployed tip.**
