# Continuation-feature proof corpus — `09153e9f1230b9cbc93f48cbcb1ac41d95391de3`

**SHIPPED 2026-06-09** — this SHA is the current PR-presentation head (`frond-scribe-claude/20260509/narrow-surgery-tight`). It is the **history-preserving back-merge** of the proven fold `8b5dde6165` + `upstream/main` `7cdec28706` (makes the PR mergeable so upstream CI runs).

> **CARRY-OVER NOTICE:** This is the full proof set carried forward from `PROOFS/8b5dde6165…/`. The carry-over is **byte-valid** — every continuation-primitive prod surface the rows attest is byte-identical between `8b5dde6165` and `09153e9f12`. See **`ADDENDUM.md`** for the back-merge drift-delta, the code-path byte-identity table, and the safety-byte verification. `R-CD-3` re-points (run.ts byte-unchanged); all other rows' evidence applies unchanged.
