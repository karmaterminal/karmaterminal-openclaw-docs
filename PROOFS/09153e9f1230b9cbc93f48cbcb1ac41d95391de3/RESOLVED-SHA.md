# RESOLVED-SHA — `09153e9f1230b9cbc93f48cbcb1ac41d95391de3`

> **Back-merge head** = `8b5dde6165` (proven fold) + `upstream/main` `7cdec28706`, history-preserving. The SHA-identity + gate verdicts below are inherited from the `8b5dde6165` parent; all continuation-primitive code-paths are byte-identical (see `ADDENDUM.md`). Safety bytes re-verified on `09153e9f12`: compactionFailureContext=0 (run.ts byte-unchanged), slack cure 4, matrix cure 3.
