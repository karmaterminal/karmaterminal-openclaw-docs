# R-CW-1: continue_work nominal + chain-counter persist at ship-SHA 1cfd285ad1

**Seat**: cael (🩸)
**Build**: OpenClaw 2026.6.2 (e66dc63) — running gateway SHA-confirmed `1cfd285ad1`, Main PID 3238078, gateway restarted 2026-06-08 07:16:31 PDT (fresh deploy, long loop cut).
**Date**: 2026-06-08 07:34 PDT
**Candidate**: continuation ship-candidate at `1cfd285ad1` (#952 lineage / #85651 surface)
...
```
continue_work(delaySeconds=5, reason="R-CW-1 cure/continuation PROOF fire at ship-SHA 1cfd285ad1: nominal continue_work tool call at 5s delay, certifying wake + chain-counter persist on cael-seat live gateway ...")
```
...
2. **`continuation.work`** span (parent = `openclaw.message.processed` root) with:
   ...
   - `reason.preview: "R-CW-1 cure/continuation PROOF fire at ship-SHA 1cfd285ad1: nominal continue_work ..."` — matches the fire.

## Verdict

✅ **PASS** — `continue_work` tool-form fires at ship-SHA `1cfd285ad1`: schedules (`status=scheduled`), nominal delay reported without clamp (`delaySeconds=5`, no `note`), traceparent emitted, and the **chain-counter persists at the byte** (`chain.step.remaining=199` vs maxChainLength=200, anchored to `chain.id 67b3e80a...`). Wake + chain-counter-persist primitive certified live on cael-seat.
