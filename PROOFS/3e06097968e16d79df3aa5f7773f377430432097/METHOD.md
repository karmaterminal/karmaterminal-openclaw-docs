# Method — Project 81 5292 proof rerun transposed to 3e06097

## Scope

- **Safe assembly / corpus SHA:** `3e06097968e16d79df3aa5f7773f377430432097`
...
2. Seed `PROOFS/3e06097968e16d79df3aa5f7773f377430432097/` from the previous full-copy corpus so corpus-dependent rows have an addressable target.
...
7. During proof generation and PR-presentation preparation, absorb current upstream/main drift into the safe assembly branch, validate the touched non-proof-row drift slices, and full-copy transpose the 5292 corpus to push SHA `3e06097968e16d79df3aa5f7773f377430432097`. The final 3e06097 drift batches were Android screenshot/notification text determinism and UTF-16 safety, macOS launch-at-login stability, Codex/channel/auth/OpenAI/native-hook/tool-policy/gateway/tool-result UTF-16-safe truncation, provider catalog refreshes for Qwen/Cohere/Mistral, UI split-view toolbar unification, native app i18n inventory refresh, Slack Enterprise Grid channel behavior, serialized tool-payload UTF-8 byte-limit handling, and the cleanup-safe TUI PTY concurrency refactor. The single TUI conflict was resolved by keeping upstream cleanup registration while preserving the safe-assembly gateway `/stop` abort path and extended PTY timeout budget.
...
This corpus does not claim a second live gateway proof run at 3e06097. It preserves `proof_source_sha=5292af40d0ad5303b85a678f6e629503a8725848` and `proof_push_sha=3e06097968e16d79df3aa5f7773f377430432097` because the final drift corrections were freshness/ancestry take-backs that did not modify the continuation proof-row behavior under test.
