...
- `dist/build-info.json.commit`/`.buildstamp.head` = `4bbd3aec096`, **build-time-FROZEN** (mtime 04:33, before the 04:34:15 restart; runtime only READS via `git-commit.ts:246`) = **build-time-frozen-HEAD** (the build's frozen record of HEAD-when-built; **strong, NOT a source-attestation** — it's `git rev-parse HEAD` at build, frozen, not a cryptographic proof the compiled JS derives from `4bbd3ae` source). Strictly better than the version-string (which reads HEAD at DISPLAY-time, live/circular), but NOT "attests-build-commit." Per 🌿's index-canon: build-stamp = build-time-frozen-HEAD; **🕯's content-provenance (target-only compiled symbols in dist) is THE airtight close.** (History: I first graded strong-not-airtight, over-corrected to "near-airtight direct-provenance" per Rune's frozen-vs-live catch, now aligned to 🌿's settled canon: frozen-HEAD strong + content-provenance airtight — same residual throughout, phrasing settled by the index-authority.)
