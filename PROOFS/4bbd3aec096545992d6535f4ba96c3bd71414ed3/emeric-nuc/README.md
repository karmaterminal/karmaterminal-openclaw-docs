...
- `compactionFailureContext` literal grep src/+dist/ = **0** = clean Form-B upstream-faithful state (NOT the count-4 catastrophe). **Referent — frond's FINAL pin (`1514253204`, supersedes ALL her prior cfc pins), landed by byte-elimination:** `compactionFailureContext` is frond's harness-shorthand (greps to 0, not a source symbol); the "0/5 never 4" invariant resolves to **(a) the continuation-tool-REGISTRATION count** — 5 continuation-tool-classes register or none, never the partial-4 silent-drop (#917/#918/#920). frond accepts (a) **on the byte-elimination** (stronger ground than her original name-fit), with the standing caveat that it's her harness-shorthand + **figs can override; non-blocking either way (every surface green).** The elimination that left (a) as the sole literal-fit:
  - **queued-count ❌** — `post-compaction-delegate-dispatch.test:634` asserts `{queuedDelegates:4, droppedDelegates:1}` VALID (an *accounted* budget-reduction when a bracket already spawned, NOT a silent under-stage). A literal "never-4" invariant cannot live in a count the test itself sets to 4. (Rune's semantic "never-*silently*-under-staged" reading would survive `:634`, but frond did not land there — she landed on (a).) Off-table as the literal referent.
  - **seat-count ❌** — it's N=**6** now (Elliott joined), not 5; a seat-count invariant would be "0/6", not "0/5". The cohort-size-INDEPENDENCE of the "5" is the tell it counts tool-classes, not seats. (Seat cross-walk {0,6} = Elliott's R-OBS-1, SEPARATE from cfc — agreed.)
  - **→ tool-registration (a)** — the SOLE candidate fitting the literal "0/5 never 4": fixed at 5 tool-classes regardless of cohort size, never-free-4 (all-register-or-partial-drop). And it **PASSES** on `4bbd3ae`: emeric `R-CONTINUATION-TOOL-REGISTRATION.md` misconfig-warn 12/12 + all-3-families register; Ronan 6/6 full-set. ✅
  The deployed-`0` is the clean Form-B state; the gate is GREEN. **Referent = (a) tool-registration per frond's final pin (figs may override; non-blocking).** (Volitional-surface #2 also intact/clean — omit-at-zero by design `status-message.ts:117-118`, reads 0 — and cross-walk #3 = R-OBS-1 6/6; both green, both separate from the cfc-label.)
