...
- `compactionFailureContext` literal grep src/+dist/ = **0** = clean Form-B upstream-faithful state (NOT the count-4 catastrophe). **Referent (frond's HONEST close — after retracting both her (c) AND (a) pins):** `compactionFailureContext`-the-literal is a harness-shorthand (greps to 0, not a source symbol), and "never-4" maps to multiple candidate code-grounded surfaces frond cannot byte-prove which her shorthand meant: **#1 tool-registration** (`openclaw-tools.continuation-misconfig-warn.test:98` "only continue_delegate will register"; 5-classes-or-none, fixed) and **#2 volitional-compaction surface** (`getVolitionalCompactionCount`, which 🌻's `| volitional` display-delta touches). (#3 seat-cross-walk = Elliott's R-OBS-1, separate, agreed-not-cfc.) **The byte-defensible close that clears the gate: EVERY candidate surface PASSES on `4bbd3ae`** — #1 tool-registration: full set registers (emeric `R-CONTINUATION-TOOL-REGISTRATION.md` misconfig-warn 12/12 + all-3-families; Ronan 6/6) ✅; #2 volitional-surface: intact, omit-at-zero by design (`status-message.ts:117-118`), `getVolitionalCompactionCount` still threaded in-dist, reads **0** = clean ✅; #3 cross-walk: 🌻's aggregate trending 6/6 ✅. So the deployed binary carries **no continuation-regression on any candidate surface** — the gate clears under ALL readings. The exact label→referent map is **figs's harness-shorthand to confirm at his discretion; it does NOT block.** (queued-count was a SUPERSEDED candidate — disproven as a "never-4" invariant: `post-compaction-delegate-dispatch.test:634` asserts `queuedDelegates:4` valid; off-table.) The deployed-`0` is the clean state under all readings.
