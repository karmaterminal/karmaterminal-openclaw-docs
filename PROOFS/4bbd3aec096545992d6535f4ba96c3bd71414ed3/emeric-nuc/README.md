...
- `compactionFailureContext` literal grep src/+dist/ = **0** = clean Form-B upstream-faithful state (NOT the count-4 catastrophe). **Referent (frond's FINAL pin):** `compactionFailureContext` = the continuation-**tool-registration** count — `{0,5} never 4`, a fixed tool-class invariant of the binary (NOT seat-count, NOT queued-count; both candidate-readings byte-disproven — queued-count allows 4 at `post-compaction-delegate-dispatch.test:634`). **5** = continue_work + continue_delegate + request_compaction all register at spawn-init; **4** = a sibling silently drops (#917/#918/#920 half-symmetric regression, `openclaw-tools.continuation-misconfig-warn.test:98` "only continue_delegate will register"); **0** = clean Form-B. emeric's proof of the pinned referent: `emeric-nuc/R-CONTINUATION-TOOL-REGISTRATION.md` (misconfig-warn 12/12 + all-3-families-register on `4bbd3ae`). The deployed-`0` here is the clean-Form-B PASS state for that invariant. (Distinct + separate from Elliott's R-OBS-1 cross-walk-completeness N=6, which is a different real invariant, not cfc.)
