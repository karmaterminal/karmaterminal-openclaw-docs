...
## 🔧 GRADE CORRECTION: MECHANISM ✅ PROVEN + execution-gated-by-the-RETRYABLE-guard (2026-06-10 09:25 PDT — tightened from an earlier over-reach to FULL PASS, per 🌿+🕯 byte-discipline)

**#952/#959 = no-cancel MECHANISM ✅ PROVEN (the hard part — the historical-bug cure — GREEN) + full turn-EXECUTION gated by the expected RETRYABLE in-flight guard, full-execution capturable on a quiet seat.** CORRECTION: an earlier note graded this "FULL PASS" — that was a half-byte ahead (caught by 🌿+🕯, `1514304431`). The byte-honest line: the retryable-guard proves execution isn't *permanently* gated, but that's INFERENTIAL ("the guard would let it through in quiet conditions"); the runbook bar is EMPIRICAL — a hop-2 TURN actually in the jsonl. Inferring-execution ≠ observing-it. So the grade is mechanism-proven + execution-gated-by-the-retryable-guard, NOT FULL PASS. The `requests-in-flight` guard is **RETRYABLE** (not a permanent gate): Byte (verified at `4bbd3aec096`):
...
**Grade: #952 — no-cancel MECHANISM ✅ PROVEN** (parse `bracketIdx=0` + arm + fire + wake hop=4/200 + **not-cancelled** = the historical-bug cure, the hard part, GREEN) **+ full turn-EXECUTION gated by the expected RETRYABLE in-flight guard** (`work-dispatch.ts:82-83`/`:266-268` — the runbook's own quiet-seat precondition, byte-confirmed temporary, NOT a defect/gap). Full-execution (a hop-2 turn in the jsonl) is **pending/capturable on a quiet seat** — the clean runbook close if a quiet window opens on rune-seat. The `:83` retryable-byte makes the fallback-grade byte-DEFENSIBLE ("execution-gated-by-the-expected-RETRYABLE-guard, the documented precondition") rather than "execution unobserved/unknown" — that's the difference between "we couldn't prove it" and "it's gated by exactly the documented expected-behavior." Cohort byte-discipline: 🕯 (retryable-guard byte `1514302187`, then self-tightened from FULL PASS `1514304431`), 🌿 (tightened the grade), 🪨 (byte-verified the guard `1514347767`, corrected this doc from the FULL-PASS over-reach). The detailed journey-grade below is preserved as the evidence-trail.
