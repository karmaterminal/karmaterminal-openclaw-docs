...
Behavioral proof corpus for the **2026-06-08 deployed candidate SHA** — the fleet-live assembly head (all six seats deployed `7992640e60`, fresh gateways, the long loop cut). This corpus certifies the **runtime-half** the cohort honestly flagged open during the source-GO adjudication (Q1 keep / Q2 strip / structural-by-threading): the continuation/delegate runtime path, RUN live on the deployed bytes — the byte RUN is the certification, not the bank.

- **SHA**: `7992640e60` (`OpenClaw 2026.6.2`)
- **Deploy**: all six seats live on `7992640e60` (cael/ronan/emeric/rune/elliott direct; silas via elliott-build+rsync), fresh gateways restarted ~07:16–07:21 PDT.
...
Every continuation-tool fire captures the Grafana Tempo trace (`http://tempo.dandelion.cult/api/traces/<id>`) + span export, per figs's 2026-05-16 directive. All ronan-seat R-CD traces captured on deployed gateway **pid 1581565** (the live `7992640e60` deploy).

## Cross-session targeted-return: RESOLVED (the 7-reversal cascade, settled at the byte)

**Final shape (Ronan's decisive byte-walk + Silas's RUN, cohort-converged):** cross-session targeted-**RETURN**-routing delivers **UNCONDITIONALLY** on `7992640e60` (no policy-gate, durable-enqueue-for-activation). **Silas's intersession sub-row = genuine cross-session RETURN PASS, config-INDEPENDENT.**
...
- **R-CD-3 (post-compaction lifeboat):** involves the compaction path — assigned to Ronan/Emeric for re-run with exact-SHA traces on `7992640e60`.
...
**Re-pointed from `7992640e60` → assembly-backmerge tip after PR #966 merge.**

The current assembly-backmerge tip is `7992640e60` (commit: "fix(ci): resolve lint/types/deadcode trivials on bd4276c813"). This is `bd4276c813` + 3 test/lint/deadcode-allowlist fixes — all test-only, **runtime byte-identical** to `bd4276c813` on every code path.

**Re-point validity:** All rows that re-pointed cleanly from `e66dc63f` → `bd4276c813` also re-point cleanly from `bd4276c813` → `7992640e60` (the trivial CI fixes don't touch any runtime path). R-CD-3 (post-compaction lifeboat) still requires RE-RUN per the previous re-point note — the run.ts compaction-rotation change at `bd4276c813:2095` is still the only behavioral delta from the original `e66dc63f` proof-basis.

---

## RE-POINT NOTE 3 (2026-06-09 00:02 PDT, Silas)

**Re-pointed from `ae5e01e76f` → `7992640e60`** (PR #969 merged: Rune's gate-validation code-agent caught a duplicate `updatedAt: Date.now()` lint error in `subagent-announce.continuation.test.ts:184-186` — collision artifact from PR #966 + Elliott's parallel manual fix at `22ebe7f296`).
...
**Re-pointed from `f3a411ad` → `7992640e60`** (PR #970 merged: Rune's lane-artifact cleanup — removed WORKORDER.md + tmp-drop-me-rune-gate.md that accidentally landed via PR #969).
...
Chain: `e66dc63f` → `bd4276c813` → `ae5e01e76f` → `f3a411ad` → `7992640e60`

---

## RE-POINT NOTE 5 (2026-06-09 03:32 PDT, Silas)

**Re-pointed from `1cfd285ad1` → `7992640e60`** — Form B `compactionFailureContext` removal landed on assembly-backmerge as the new tip (run.ts upstream-faithful timeout-compaction failover, count-0, byte-faithful to upstream).

**Why RE-POINT (most rows) vs RE-RUN (R-CD-3):** The dispatch/return-routing paths (R-CW-*, R-CD-1/2/4, R-RC-*) are byte-identical between `1cfd285ad1` and `7992640e60` on every code path EXCEPT the compaction-failure-rotation path that Form B touches. R-CD-3 (post-compaction lifeboat) is the one row that genuinely RE-RUNs (Emeric's 0616 cron handles this).

**Note: this re-point assumes Form B becomes the canonical ship-SHA after figs's 0600 pick.** If figs picks Form A: the assembly-backmerge tip will revert to `1cfd285ad1` (count-5 overload), and these proofs effectively re-point back to the prior PROOFS dir (which is preserved at commit `65bae83`).

Chain: `e66dc63f` → `bd4276c813` → `ae5e01e76f` → `f3a411ad` → `1cfd285ad1` → `7992640e60` (Form B staged-landed)
