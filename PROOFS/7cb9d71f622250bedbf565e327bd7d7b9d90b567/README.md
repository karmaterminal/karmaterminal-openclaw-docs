...
- after: 24 PASS / 7 PARTIAL / 1 FAIL / 6 MISSING

Exactly three rows moved from MISSING to PASS:
`R-CW-MULTI-COLLAPSE`, `R-CW-7`, and `R-CW-DELEGATE-CHILD-LIVE`.
...
`R-CD-COLLECTION-ON-COLLAPSE` remain MISSING. Every other baseline row state
is unchanged.
