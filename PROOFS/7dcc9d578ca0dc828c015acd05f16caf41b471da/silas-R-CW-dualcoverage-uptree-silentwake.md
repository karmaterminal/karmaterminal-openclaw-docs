...
**Transfer basis:** `subagent-announce.ts` (the exercised code — delegate-dispatch + spawn + return-wake) is **byte-identical** between `e66dc63f` and `7dcc9d578c` (Gate 2 cure-bytes, 14/15 cores unchanged).
...
Same basis as sub-row 2: `subagent-announce.ts` is byte-identical in the 14/15 cores set. The dispatch+spawn+return-wake mechanism is unchanged. The runtime-proven PASS on `e66dc63f` applies identically to `7dcc9d578c`.
