...
## Both-forms note (bracket-form half) — resolved at the byte

The BOTH-FORMS MANDATE (#952) calls for proving the cross-walk via BOTH the typed `continue_work()` tool AND the `CONTINUE_WORK` bracket/token form. **Tool-form: PROVEN above.** The bracket-form's disposition is now resolved at the byte (see `emeric-nuc-crosswalk.md` in this dir + `../R-CW-DELEGATE-TOKEN/`): the bracket parse IS alive on `8cafdcd` (gateway continuation-log byte: a bare `CONTINUE_WORK:5` parses `bracketIdx=0 kind=work origin=bracket`), but a fresh seat-trace is emission-surface-gated on emeric-main (message-tool-only delivery, per 🌊's R-CD-TOKEN) and the subagent path is design-guarded (`subagent-announce.ts:977`, per my R-CW-DELEGATE-TOKEN / #952 finding). The populated-vs-absent contrast carries from the exemplar `077b261dd8` (`reason.preview` absent on the bracket form) + the live parse-byte. No fabricated bracket-trace — byte-honest.
