...
- **partial (4):** R-CW-DELEGATE-TOKEN (#952 — CONTESTED-DEFERRED, figs's RFC: ROW bar L86 = lightContext SUBAGENT NO-tool fires bare/bracket CONTINUE_WORK:N in a continue_delegate child on a dormant seat -> hop-2 must EXECUTE; code-confirmed ship 8cafdcd that subagent CONTINUE_WORK is IGNORED at subagent-announce.ts:977 — so spec-bar vs :977-ignore CONFLICT on this exact row. Distinct from R-CD-TOKEN [subagent CONTINUE_DELEGATE -> :1089 spawns, CLOSED] and R-CW-TOKEN [main-session reply-text, line 84]. :977-ignore = FAIL-vs-by-design is figs's call; actual lightContext-subagent surface UNEXERCISED) · R-CW-TOKEN (PARSE proven, main-session hop-2-EXECUTION-DRIVE owed) · R-CD-CHAINED-DEPTH-2 (TEST-1/2 filled, TEST-3 echo-broadcast owed) · R-RC-2 (ACCEPT proven cross-seat exemplar, fresh-`8cafdcd` executed-accept owed)
