...
- **partial (4):** R-CW-DELEGATE-TOKEN (#952 — CONTESTED-DEFERRED, figs's RFC: ROW bar L86 = lightContext subagent NO-tool fires bare/bracket CONTINUE_WORK:N in a continue_delegate child on a dormant seat -> hop-2 must EXECUTE; UNEXERCISED. #952's actual MECHANISM = figs's-mandate-vs-code-walk OPEN: token-per-L71-tool-blind vs continue_work-tool-call-timer-per-code-walk; figs's call) · R-CW-TOKEN (PARSE proven, main-session hop-2-EXECUTION-DRIVE owed) · R-CD-CHAINED-DEPTH-2 (TEST-1/2 filled, TEST-3 echo-broadcast owed) · R-RC-2 (ACCEPT proven cross-seat exemplar, fresh-`8cafdcd` executed-accept owed)
