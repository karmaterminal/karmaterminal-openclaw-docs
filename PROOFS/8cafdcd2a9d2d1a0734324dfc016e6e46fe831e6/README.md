...
- **partial (4):** R-CW-DELEGATE-TOKEN (#952 — CONTESTED-DEFERRED, figs's RFC. Two layers un-conflated: (A) the PROOF-CORPUS ROW [spec L86] = lightContext SUBAGENT NO-tool fires bare/bracket CONTINUE_WORK:N in a continue_delegate child -> hop-2 must EXECUTE = the both-forms TOKEN-half; (B) #952 THE ISSUE's repro [firsthand] = the continue_work() TOOL-call self-chain [continue_work(delaySeconds=0)] -> dies after hop 1. #952's MECHANISM is UNSETTLED in the issue itself = figs's RFC: top-root = heartbeat flood-guard, but the issue's own Exonerated list calls it a harness-artifact + carries a re-entry-wiring/#954 framing. Flood-path UNEXERCISED; the :977 subagent-announce decline [code-confirmed 8cafdcd] is an ADJACENT path, NOT the tool-call repro. R-CD-TOKEN [subagent CONTINUE_DELEGATE -> :1089 spawns] CLOSED + distinct; R-CW-TOKEN [main-session, line 84] distinct) · R-CW-TOKEN (PARSE proven, main-session hop-2-EXECUTION-DRIVE owed) · R-CD-CHAINED-DEPTH-2 (TEST-1/2 filled, TEST-3 echo-broadcast owed) · R-RC-2 (ACCEPT proven cross-seat exemplar, fresh-`8cafdcd` executed-accept owed)
