...
## CAPABILITY-MATRIX cross-ref (figs's both-forms directive, 2026-06-09)
See `CAPABILITY-MATRIX-tool-vs-token.md` (this dir) for the full tool-vs-token deviation matrix. KEY finding for R-CW-TOKEN: continue_work token-form is **non-functional in subagent CHAIN-HOP** (`subagent-announce.ts:975` ignores it) → #952/#958 — the tools-denied failure surface this row's both-forms mandate exists to catch. The main-channel + subagent-turn-1 token-drive works (proven above); the chain-hop is the broken cell.
