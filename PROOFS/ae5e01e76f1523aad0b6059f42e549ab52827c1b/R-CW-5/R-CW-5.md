# R-CW-5: cost-cap rejection at dispatch time — ship-SHA ae5e01e76f
...
```
2026-06-08T07:49:01.791-07:00 [continuation/delegate-dispatch] [continuation:delegate-rejected] cost-capped task=R-CW-5 cost-cap rejection PROOF at ae5e01e76f (cael-seat, costCapTokens=100): ... session=agent:main:discord:channel:1466192485440164011
```

- Rejection reason: **`cost-capped`** — this build distinguishes the cost-cap dimension explicitly (prior cure-builds emitted unified "chain-capped"; ae5e01e76f names `cost-capped`).
...
✅ **PASS** — chain-budget cost-cap enforcement fires at dispatch time on cael-seat at `ae5e01e76f`: tool returns `scheduled` (optimistic), dispatch emits `[continuation:delegate-rejected] cost-capped`, no `continuation.delegate.dispatch` span, no wake delivered, task body suppressed. Cost-cap certified against the seat-specific cap (forced 100; baseline 500000 restored post-row).
