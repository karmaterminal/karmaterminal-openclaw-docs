...
**Seat:** silas / lothric — gateway restarted 07:21 PDT onto `bd4276c813`, healthy
...
2. 1 run.ts prod-fix at `:2095` (compaction-failure-rotation path)
3. 1 whatsapp test-fixture fix (test-only)

None of these touch the **continuation-delegate silent-wake dispatch/return path** this proof tests (`continue_delegate(mode=silent-wake)` → dispatch → spawn → silent return → wake). The dispatch machinery, silent-return-routing, and wake-trigger are byte-identical between the two SHAs. The proof evidence holds verbatim.
