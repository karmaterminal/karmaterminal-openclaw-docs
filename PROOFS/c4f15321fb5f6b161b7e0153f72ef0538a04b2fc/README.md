...
| R-CD-4 (targeted RETURN routing) | 🌊 ronan | `targetSessionKey` routes the delegate RESULT (same-session scope) | ⏳ dispatched (receipt in `R-CD-4/`); runtime Delivered-log capture pending follow-up turn |
| R-CD-CHAINED-DEPTH-2 (depth-2 tree-broadcast) | 🌊 ronan | depth-2 chain + `fanoutMode=tree` return-to-ancestors | ⏳ dispatched (receipt in `R-CD-CHAINED-DEPTH-2/`); leaf-return capture pending follow-up turn |
...
- **R-CD-4 / R-CD-CHAINED-DEPTH-2 verdicts rest on the runtime Delivered-log + Tempo, NOT the tool-surface echo.** The dispatch receipts (`dispatch-trace.txt`) record the scheduled-status + echoed routing fields (necessary-not-sufficient); the EVIDENCE.md verdict-upgrade lands when the runtime `[continuation:targeted-return] Delivered` / depth-2 leaf-return is captured on the follow-up turn.
