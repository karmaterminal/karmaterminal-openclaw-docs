...
**SHIP SHA (LOCKED):** `c8149791797eeefec1a7e6479c800bef51cac9f8`
...
**Captured:** 2026-06-21T08:42Z · **Re-homed:** 2026-06-21T08:54Z

> **RE-HOME NOTE:** Re-homed to the final locked token-fixed ship-SHA
> `c8149791797eeefec1a7e6479c800bef51cac9f8` (6/6 deployed per frond's GATE-4 GO).
> The original cross-walk already confirmed all 6 seats running git
> `c8149791797` — evidence carries over directly; this is the unaffected
> `/status` continuation cross-walk row (not the token-form). **Verdict: PASS —
> continuation feature visible fleet-wide on the deployed locked ship-SHA
> `c8149791797`.**

## Claim

The continuation feature (`continue_work`, `continue_delegate`, `request_compaction`) is
deployed and observable fleet-wide on the LOCKED SHIP SHA `c8149791797`, both via local
`/status` / `session_status` surfaces and via external OpenTelemetry continuation spans in
Tempo.
...
### SHIP SHA provenance
`c8149791797eeefec1a7e6479c800bef51cac9f8` is the **final locked token-fixed ship-SHA**
(6/6 deployed per frond's GATE-4 GO), superseding the pre-token-fix
`93ace21341bf13a08f9bf75791f8ac70cf9542a5` (subject: `chore(plugin-sdk-surface): regen
budgets to merged-surface counts (our feature's added exports)`), which is a **verified
ancestor** of this locked ship-SHA. The deployed gateway git HEAD on all six seats **is**
`c8149791797`, so the running build is the locked ship-SHA itself and contains the
continuation feature directly. Re-confirmed on the elliott seat at re-home time:
`git rev-parse HEAD` = `c8149791797eeefec1a7e6479c800bef51cac9f8`, `openclaw status` Git =
`git · @ c8149791`, app `2026.6.9`.
