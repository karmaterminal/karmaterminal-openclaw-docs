...
The byte RUN is the cert — and Silas's RUN of the genuinely-cross-session case beat my code-read of it. Two corrections to this row's claim (the `2807efc` tool-surface-echo over-claim + this `e66dc63f` same-vs-cross-session mis-scope); both caught by running it, both owned.
