...
Captured from `journalctl --user -u openclaw-gateway` (full causal block in `targeted_return_delivery_journal.txt`), gateway pid 1581565, live `f3a411ad`:
```
07:43:10.248 [continuation:delegate-spawned] hop=5/200 mode=silent session=… task=[R-CD-4 RETURN-ROUTING certification child | echo-token: RCD4-f3a411ad]
07:43:13.742 RCD4-RETURN echo=RCD4-f3a411ad child→target routed          ← leaf's parroted echo (DELIBERATELY NOT the evidence)
...
- Resource: `process.pid=1581565` (the deployed gateway on `f3a411ad`), 15 batches.
...
The byte RUN is the cert — and Silas's RUN of the genuinely-cross-session case beat my code-read of it. Two corrections to this row's claim (the `2807efc` tool-surface-echo over-claim + this `f3a411ad` same-vs-cross-session mis-scope); both caught by running it, both owned.
