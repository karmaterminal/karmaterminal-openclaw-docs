...
**✅ PASS** on `f3a411ad`. The #952 bracket/token-form delegate self-continuation is proven by live execution: the delegate emitted `CONTINUE_WORK:7` as text and the wake delivered hop-2 (after a saturation queue-drain delay), which executed and posted.
