...
### 0.3 Stabilization Swim Doctrine

- **Fresh artifact = fresh swim.** New commit, new deploy, row 1 again.
- For **stabilization / pre-ship** swims, prefer the **full board unless genuinely blocked**. A partial slice can be good evidence, but it is not totality.
- Partial swims are more acceptable for exploratory / half-born implementation work, where the question is "what does this do?" rather than "what all is wrong with this jacket?"
- If a swim exposes a method error in `SWIM/`, patch the runbook the same cycle so future copies do not inherit the bad reflex.
...
cat ~/.config/openclaw/openclaw.json | jq '.agents.defaults.continuation'
# Expected: { "enabled": true, ... }
```

**Critical config keys** (all in `openclaw.json` under `agents.defaults.continuation`):
...
13. **A useful slice is not the whole jacket.** For stabilization swims, full-board reads beat first-fail myopia unless a real blocker makes totality too expensive.
14. **Patch the method when the method is wrong.** If chat discovers a runbook error, fix `SWIM/` before the lesson turns back into folklore.
