...
This dashboard tracks the execution outcomes of Project 81 candidate runs, providing visibility into the k6 PROOFS metrics contract defined in PR #110.
...
To keep the v1 implementation simple and avoid over-engineering with Prometheus/OTel infrastructure:
* **Source format:** Static `row-result.json` artifacts generated by `postprocess-k6-summary.mjs`.
* **Grafana Integration:** We will use the **Grafana Infinity Data Source plugin** (or standard JSON plugin). 
* **Workflow:** Grafana directly ingests the JSON artifact files via a simple HTTP fetch from the artifact directory (e.g., served by a local static HTTP server or an internal artifact bucket). The Infinity plugin parses the JSON arrays/objects natively and maps them directly into tabular formats for the dashboard panels.