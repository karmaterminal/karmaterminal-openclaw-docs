...
Reports and metrics are mandatory. Any execution, trace, lineage, parser, evidence, sanitization or publication failure overrides a stale PASS and fails the matrix. Prometheus and OTLP files are written into one immutable generation and become visible together through one atomic generation-pointer swap; each file carries that generation identity, and multi-file consumers resolve that pointer once and read only the pinned generation. Local metric artifacts publish before an OTLP push; a failed push invalidates those artifacts. Report failure attempts every row's invalidation and records cleanup defects without skipping later rows. Gateway journal acquisition remains private. Public journal evidence parses supported structured key/value or JSON records after an exact nonce/run/task/flow/trace/tool-call match and emits only field-specific allowlisted values and identity fingerprints; payload-bearing, credential, auth, token, secret, password, and API-key fields are dropped. It never republishes a matched source line. Session-only, keyword-only and free-text mentions are insufficient. Candidate envelopes carry a signed digest allowlist, and each referenced trace/correlation artifact must be a listed regular file with its required schema. Both metrics input modes require consistent row identity and signed process authority for protected rows, with the signed and metadata run IDs matching the authoritative run directory (the parent of CW7's prerequisite directory).

Dependency receipts use `openclaw.k6.signed-producer-dependency.v1` and the canonical signed-observer mechanism. Configure an operator-controlled `OPENCLAW_PRODUCER_TRUST_FILE` (issuer-to-key JSON, private, never a receipt-provided key), `OPENCLAW_PRODUCER_RUN_ID`, and `OPENCLAW_PRODUCER_RECEIPTS`. The signature binds row, exact product/runtime/docs SHAs, run, evidence identity, artifact digests and a maximum one-hour validity interval. After validating the entire bundle, the resolver atomically burns it in the verifier-selected per-user durable store outside output roots and immutable harness snapshots. The store location is not caller-configurable; its owner/lease record supports bounded stale-lock recovery. Unsigned objects, duplicate rows, partial bundles, cross-root attempts, alternate-store attempts, and replayed receipts cannot unblock dependencies.

The process observer signs and independently validates a terminal receipt containing exact candidate tree, docs SHA, expanded argv/script digests, suite exit and required checks. Product Git identity checks discard every inherited `GIT_*` control variable and require one canonical worktree root. Product tests run from a fresh detached exact-candidate clone under the verifier's resolved Node/Corepack pair after the SHA512-pinned package manager performs a frozen dependency installation; final HEAD, tree, cleanliness, dependency graph, Vitest shim, and repository-owned runner bytes are authenticated before PASS is sealed. Caller-selected Node and Corepack executables are ignored. `run-result.json` binds the validated receipt digest. Set a private `OPENCLAW_PROCESS_RECEIPT_KEY` for later independent report/metrics verification; otherwise the runner uses a process-scoped random key and later unverified receipts remain non-PASS. The suite child never receives that key. Missing exact private run/trace or parser/run attribution is non-PASS: a time-window or shared-trace match does not replace those joins.
