...
function reconcileReview(review, label, { complete = false } = {}) {
  if (!review || typeof review !== 'object' || Array.isArray(review)) {
    throw new Error(`R-CD-2 ${label} review state is required`);
  }
  if (
    review.status !== 'ready-for-human-review' ||
    !Array.isArray(review.pendingReceipts) ||
...
function reconcileAuthorityPolicy({ manifest, runResult, envelope, rowResult }) {
  if (
  ...
  if (runResult) reconcileReview(runResult.review, 'run result');
  if (rowResult?.review) reconcileReview(rowResult.review, 'normalized row result');
  if (envelope) {
    if (
    ...
    reconcileReview(envelope.review, 'candidate envelope', { complete: true });
  }
}
...
export function isRcd2AuthorityRequired({
  ...
  if (values.some((value) => String(value || '').toUpperCase() === R_CD_2_ROW)) return true;
  const scenarios = [
