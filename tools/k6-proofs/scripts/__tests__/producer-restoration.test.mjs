...
test('live producer lifecycle rejects failed and replay-invalid end events', () => {
  for (const data of [
    { phase: 'end', status: 'failed' },
    { phase: 'end', status: 'ok', replayInvalid: true },
    { phase: 'error' },
  ]) {
...
test('multi rejects duplicate flows and repeated/multiply-labelled wakes', async () => {
  ...
  const evidence = {
    ...
    wake_runs: {
      one: { startedAt: 1, endedAt: 2, labels: ['immediate', 'delay-60'] },
      two: { startedAt: 1, endedAt: 2, labels: ['delay-120'] },
      three: { startedAt: 1, endedAt: 2, labels: ['delay-120'] },
    },
  };
