...
if (args.row === 'R-CD-2') {
  ...
  try {
    ...
    correlation = args['lifecycle-correlation']
      ? JSON.parse(readFileSync(args['lifecycle-correlation'], 'utf-8'))
      : null;
  } catch {
    console.error('ERROR: R-CD-2 lifecycle receipt or correlation is unreadable');
    process.exit(1);
  }
...
const publicManifestIdentity = args.row === 'R-CD-2'
  ? projectRcd2PublicManifest({ candidateSha: args.sha, seat: args.seat })
  : null;
const outDir = join(
  'PROOFS',
  args.sha,
  args.row,
  publicManifestIdentity?.seat || args.seat,
  runId,
);
...
if (args['seat-readiness']) {
  if (args.row === 'R-CD-2') {
    const readiness = JSON.parse(readFileSync(args['seat-readiness'], 'utf8'));
    writeFileSync(
      join(outDir, 'seat-readiness.json'),
      `${JSON.stringify(projectRcd2PublicSeatReadiness(readiness, {
...
if (args.row !== 'R-CD-2' && safeEvents.length > 0) {
...
const result = args.row === 'R-CD-2'
  ? projectRcd2PublicRowResult({
      lifecycleReceipt,
      publicSummary,
      candidateSha: args.sha,
      seat: publicManifestIdentity.seat,
      runId,
      generatedAt: new Date().toISOString(),
    })
  : genericResult;
...
const genericMd = `# ${args.row} — ${args.seat} — ${verdict}
